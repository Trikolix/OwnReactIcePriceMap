...
class PrivateRouteCountEvaluator extends BaseAwardEvaluator {
    const AWARD_ID = 16;

    public function evaluate(int $userId): array {
        global $pdo;
        $count = $this->getPrivateRouteCount($userId);
    ...
    private function getPrivateRouteCount(int $userId): int {
        global $pdo;
        $sql = "SELECT COUNT(id) AS route_count
                FROM routen
                WHERE nutzer_id = ? AND ist_oeffentlich = 0";
