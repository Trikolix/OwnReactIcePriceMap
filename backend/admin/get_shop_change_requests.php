...
$openingMap = fetch_opening_hours_map($pdo, array_column($requests, 'eisdiele_id'));
...
$response = array_map(function ($row) {
    ...
    $hoursRows = $openingMap[$shopId] ?? [];
    $currentStructured = build_structured_opening_hours($hoursRows, $row['shop_opening_hours_note'] ?? null);
    ...
    return [
        ...
        'shop_opening_hours_note' => $row['shop_opening_hours_note'],
