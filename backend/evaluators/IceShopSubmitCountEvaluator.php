...
class IceShopSubmitCountEvaluator extends BaseAwardEvaluator {
    const AWARD_ID = 8;

    public function evaluate(int $userId): array {
        global $pdo;
        $count = $this->getIceShopSubmitCount($userId);
    ...
    private function getIceShopSubmitCount(int $userId): int {
        global $pdo;
        $sql = "SELECT COUNT(id)
                FROM eisdielen
                WHERE user_id = ?";
