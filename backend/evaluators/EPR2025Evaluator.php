...
class EPR2025Evaluator extends BaseAwardEvaluator {
    const AWARD_ID = 40;
    ...
    public function evaluate(int $userId): array {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM user_qr_scans WHERE user_id = :userId AND qr_code_id = 5");
        $stmt->execute(['userId' => $userId]);
        $hasScanned = $stmt->fetchColumn() > 0;

        if (!$hasScanned) return [];

        // Prüfe: Hat Nutzer ein Eis per Rad eingecheckt zwischen 07.09. und 21.09.?
        $stmt = $pdo->prepare("
            SELECT COUNT(*) 
            FROM checkins 
            WHERE nutzer_id = :userId 
              AND anreise = 'Fahrrad'
              AND DATE(datum) BETWEEN '2025-09-07 00:00:00' AND '2025-09-21 23:59:59'
        ");
        $stmt->execute(['userId' => $userId]);
