...
class OnSiteEvaluator extends BaseAwardEvaluator {
    const AWARD_ID = 38;

    public function evaluate(int $userId): array {
        global $pdo;
        $count = $this->getCheckinOnSiteCount($userId);
    ...
    private function getCheckinOnSiteCount(int $userId): int {
        global $pdo;
        $sql = "SELECT COUNT(id) AS checkins_count
                FROM checkins
                WHERE nutzer_id = ? AND is_on_site = 1";
