...
class BikeCountEvaluator extends BaseAwardEvaluator {
    const AWARD_ID = 25;

    public function evaluate(int $userId): array {
        global $pdo;
        $count = $this->getBikeCount($userId);
    ...
    private function getBikeCount(int $userId): int {
        global $pdo;
        $sql = "SELECT COUNT(id) AS checkins_count
                FROM checkins
                WHERE nutzer_id = ? AND anreise = 'Motorrad'";
