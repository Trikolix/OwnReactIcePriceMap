...
const MentionInviteModal = ({ open, onClose, checkinId, shopId, inviterName, shopName, date, userId }) => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/checkin_mentions.php?action=getStatus&checkin_id=${checkinId}&mentioned_user_id=${userId}`)
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_BASE_URL}/checkin/get_checkin.php?nutzer_id=${userId}&shop_id=${shopId}&date=${date}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleAcceptExisting = async (existingCheckinId) => {
    ...
    await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/checkin_mentions.php?action=accept`, {
      method: "POST",
  ...
  const handleDecline = async () => {
    await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/checkin_mentions.php?action=decline`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: "decline",
  ...
  const handleUndoDecline = async () => {
    await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/checkin_mentions.php?action=undoDecline`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: "undoDecline",
